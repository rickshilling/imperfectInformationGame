...
gameTraverse :: (Eq action) => (GameTree player action) -> [action] -> Maybe (GameTree player action)
...
drawGameTree :: (Show player, Show action) => (GameTree player action) -> [String]
...
putStrGameTree :: (Show player, Show action) => (GameTree player action)  -> IO ()
putStrGameTree g = putStr $ (unlines . drawGameTree) g

putStrMaybeGameTree :: (Show player, Show action) => Maybe (GameTree player action) -> IO ()
putStrMaybeGameTree Nothing = putStrLn "Nothing"
putStrMaybeGameTree (Just gt) = putStrGameTree gt

getInformationSets :: (Show player, Show action, Ord action) => (GameTree player action) -> (InformationSets action)
...
insertInfoSet :: (Show player, Show action, Ord action) => [(action, Maybe (GameTree player action) )] -> [action] -> (InformationSets action) -> (InformationSets action)
insertInfoSet forest actions infoSet = DM.insertWith DS.union (getActions forest) (DS.singleton actions) infoSet

getActions :: (Show player, Show action, Ord action) => [(action, Maybe (GameTree player action))] -> DS.Set action
getActions = Prelude.foldl (\set -> \element -> DS.union set (DS.singleton (fst element))) DS.empty

sameInfoSet :: (Show player, Show action, Ord action) => (GameTree player action) -> [action] -> [action] -> Bool
